...
def read_mongodb_loandeals_data():
    ...
    # Write to .csv file
    pd.DataFrame(elements_array).to_csv('dags/data/loandeals_table.csv')


# Read from .csv and import to SQL function
def write_loandeals_date_to_SQL(schema_name):

    # Read .csv data
    df = pd.read_csv('dags/data/loandeals_table.csv', index_col=0)
    ...
    for index, element in df.iterrows():
        sql = f"INSERT INTO {schema_name}.loandeals (id, _id, minOffer, totalBuying, periodWeeks, deleted)" \
               "VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING"

        value = (index, element["_id"], element["minOffer"], element["totalBuying"], element["periodWeeks"], element["deleted"])
...
def write_loandeals_date_to_SQL_daily(schema_name):
    ...
    if check_file_exists(bucket, filename, s3):
        ...
        s3.Bucket(bucket).download_file(filename, "loandeals.csv")

        shutil.move('/opt/airflow/loandeals.csv', '/opt/airflow/dags/data/daily_updates/loandeals.csv')
        print("File downloaded on the path")

        # Check for file existence
        if os.path.isfile('dags/data/daily_updates/loandeals.csv'):

            # Read .csv data
            df = pd.read_csv('dags/data/daily_updates/loandeals.csv', index_col=0)
            ...
            df.index = assign_new_pk_to_df(schema_name, 'loandeals', df)
